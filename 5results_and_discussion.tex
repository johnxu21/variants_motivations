...
First, we present the distributions of selected metrics in Fig.~\ref{fig:stats} of stars, PRs, issues and commits. For PRs and issues (closed + open) we consider for all and for commits we count commits in the main branch. While its not surprising that the counts for mainline metrics are always higher than those of the variants, it is interesting that are quire variants also popular in stars, pull requests and issues counts. This gives us confidence that we are studying real variants as opposed to social forks. 
