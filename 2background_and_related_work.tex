...
Jiang et al.~\cite{Lo:2017} state that, although forking may have been controversial in the open source software (OSS) community, it is now encouraged as a built-in feature on \gh. They further report that developers create \emph{social forks} of repositories to submit pull requests, fix bugs, and add new features. Zhou et al.~\cite{Zhou:2020} conclude that most variant forks started as social forks
...
Focusing on variant forks in the Android ecosystem, Businge et al.~\cite{businge:2018icsme} found that re-branding, simple customizations, feature extension, and implementation of different but related features are the main motivations to create a fork for Android apps.
...
we decided to carry our own exploratory qualitative survey with variant maintainers to identify possible impediments.
